...
void setup(void) // change from main to setup void
{
    ...
    uint32_t user_buffer_size = 2048  * num_channels;
    double read_buf[user_buffer_size];
    int total_samples_read = 0;

    int32_t read_request_size = 2048 ; // READ_ALL_AVAILABLE;
    ...
    do
    {
        ...
        // pthread_create(&data_split_thread, NULL, &data_split, &read1);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        // sendto(sockfd, data, sizeof(float), 0, (struct sockaddr *)&client_addr, sizeof(client_addr));
        // sendto(sockfd, bcm1, sizeof(struct DATA_SENT), 0, (struct sockaddr *)&client_addr, sizeof(client_addr));
        // printf("[+]Data send: %f\n", buffer);
        printf("[+]Data send\n");
    }
    return 0;
